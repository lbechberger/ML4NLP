...
class EntitySet():
    def __init__(self, cache_file='entity_cache.shelve'):
        ...
        self.prefix_dict = {'dbp': 'http://dbpedia.org/ontology/', 'dc': 'http://purl.org/dc/elements/1.1/',
                            'gaf': 'http://groundedannotationframework.org/gaf#', 'w3': 'http://www.w3.org/2000/01/rdf-schema#'}
    ...
    def get_entity_info(self, entity):
        ...
        entity_info['mentioned_in'] = ks2.run_dictless_query('SELECT ?l WHERE {'+entity+' gaf:denotedBy ?l }', self.prefix_dict)
        entity_info['comment'] = ks2.run_dictless_query('SELECT ?l WHERE {'+entity+' w3:comment ?l }', self.prefix_dict)
        return entity_info
    ...
    def get_person_entity_info(self, entity):
        ...
        entity_info['altnames'] = ks2.run_dictless_query('SELECT ?l WHERE {'+entity+' rdfs:label ?l}') \
            + ks2.run_dictless_query('SELECT ?l WHERE {'+entity+' dbp:alias ?l }', self.prefix_dict) \
            + ks2.run_dictless_query('SELECT ?l WHERE {'+entity+' dbp:alternativeName ?l }', self.prefix_dict) \
            + ks2.run_dictless_query('SELECT ?l WHERE {'+entity+' dbp:birthName ?l }', self.prefix_dict)
        entity_info['birtdate'] = ks2.run_dictless_query('SELECT ?l WHERE {'+entity+' dbp:birthDate ?l }', self.prefix_dict) \
            + ks2.run_dictless_query('SELECT ?l WHERE {'+entity+' dbp:birthYear ?l }', self.prefix_dict)
        entity_info['birtplace'] = ks2.run_dictless_query('SELECT ?l WHERE {' + entity + ' dbp:birthPlace ?l }', self.prefix_dict)
        entity_info['country'] = ks2.run_dictless_query('SELECT ?l WHERE {' + entity + ' dbp:country ?l }', self.prefix_dict) \
            + ks2.run_dictless_query('SELECT ?l WHERE {' + entity + ' dbp:nationality ?l }', self.prefix_dict)
        entity_info['gender'] = ks2.run_dictless_query('SELECT ?l WHERE {' + entity + ' dbp:gender ?l }', self.prefix_dict)
        entity_info['profession'] = ks2.run_dictless_query('SELECT ?l WHERE {' + entity + ' dbp:profession ?l }', self.prefix_dict)
        return entity_info
    ...
    def get_event_entity_info(self, entity):
        ...
        entity_info['partof'] = ks2.run_dictless_query('SELECT ?l WHERE {'+entity+' dbp:isPartOf ?l }', self.prefix_dict)
        entity_info['place'] = ks2.run_dictless_query('SELECT ?l WHERE {'+entity+' dbp:place ?l }', self.prefix_dict)
        entity_info['startdate'] = ks2.run_dictless_query('SELECT ?l WHERE {'+entity+' dbp:startDate ?l }', self.prefix_dict)
        return entity_info
