...
### Feature Extraction
To train our classifier we have to extract features out of our dataset. Since our dataset consists mainly out of articles (the user representation is a set of articles and positive and negative examples are articles), our main goal is to represent an article as some kind of a feature vector. For this, we had mainly two ideas that we want to test out:

1. To represent an article as a feature vector, we have to define which features we want to use. That would limit us to a fixed amount 	  of features for each article and it is not guaranteed that each article has these features. Moreover, we would not be so good in        generalizing to unseen articles, since we can only define the features for our known dataset. 

   Due to this constraints, we thought as the first idea about a "flexible" feature vector for different articles. In the first step, we    try to extract the most important words of an article by using the TF-IDF-score. Then we create a vector for every word (word2vec)    and store these vectors as the different features of our feature vector for the different articles. In this approach, every article    has different features in his feature vector. But since these features are represented as word embeddings it would be still possible    to calculate similarities between the features of the different articles. Our classifier would then train on these feature vectors.    The possible values for our feature vectors of different articles would be an array consisting out of vectors and each vector          consists out of floating point numbers. They target positive as well as negative examples.

2. The second idea is to represent articles as a list of high-level features. Therefore we would extract the most important words of our    whole dataset and relate them to the semantics that the KnowledgeStore provides to us. For instance, if the word Trump, Merkel and    Macron are very important, we would relate these word with the help of the Knowledgestore to "Politician". Based on this procedure    we would end up with a fixed list of high-level features for each article. Our classifier would then learn, in which combinations of    this high level features the user is interested in. New samples would be classified based on this high-level features.

   A disadvantage of this approach is that we have to stick to a fixed amount of features. So problems in classifying unseen samples        could arise because it is not guaranteed that every sample has the features we have defined based on our given dataset. The possible    values here for our feature vectors of different articles would be an array consisting out of boolean values, whether a high-level    feature exists in the article or not. They target positive as well as negative examples.
