...
**Update 1** (February 5, 18:10): After the computer crashed this morning and then later accidentally having terminated one run of Random Search by accident, we are still waiting for results. In the mean time, we would like to elaborate on how we are approaching the problem of hyperparameter optimization. We have decided on using a random forest classifier since it performt best (second best in accuracy and best in runtime, which is now important due to our time constraints). We are using sklearn's RandomizedSearchCV in order to find promising hyperparameters for the classifier. Once we have results from this random search, we plan on using gridsearch over a smaller set of possible parameters around the best randomly generated set.
