...
# Results
For the final evaluation, the data split into validation and test dataset made previously was utilized. All parts of the feature and model selection described above were performed on the former, the finally best constellation of preprocessing, selection and classifier was applied on the test dataset.
![BestResultsClassifier](documentation/BestResultsClassifier.png)
On the validation data set, the Random Forest Classifier performed almost equal to its direct relative, the Extra Tree Classifier. The SVC with the RBF kernel performed slightly worse, but still with an F1 score over 0.8. The Naive Bayesian approach, even if providing the worst general performance,  yielded still acceptable results, being in its best case just 13% inferior to the best algorithm.
