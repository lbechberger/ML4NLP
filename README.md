...
### Approach
We quickly realised that a complete QA (Question Answering) system capable of parsing the whole range of complex questions ("Whose fault is the decline of the Euro, according to Mrs Moneybags?") would be beyond our scope. Inspired by the triplets that make up entries in RDF (A R P), we decided to limit the questions to the style of "Who has the relation R to Patient P?", "What is the relation between Agent A and Patient P?" and "To whom does Agent A have the Relation R?".
...
Furthermore this decision enabled the splitting of the task into 3 variants, one for each style of question. This is especially well suited for our approach, since we consist of three groups that will be working in parallel. The group epsilon decided to focus on answering questions focused on relations, i.e. of the type "What is the relation between Agent A and Patient P?" --> "R" 
...
An event that has both an agent and a patient is considered a "Relation". We will use this term, but be aware that there may be events that are not relations. However,we most likely won't be talking about these in our part.
...
From these filtered queries we desire to generate the test and training data sets. However, as mention in "Choice of Input and Target" we are not quite sure if we also want to add real world knowledge about the entries in the article via further SPARQL queries to dbpedia.
